...
(async function theme() {
        ...
        categoryOptions.flatMap(({ name, defaultVal, revealOptions }) => {
          ...
          const revealSettings =
            revealOptions?.map((option) => [
              `theme:${option.name}`,
              mainValue
                ? getLocalStorage(`theme:${option.name}`, option.defaultVal)
                : false,
            ]) || [];
  ...
  const applySettings = (settings, options) => {
    Object.values(options)
      .flat()
      .forEach(({ name, type, revealOptions, run }) => {
          ...
          revealOptions.forEach(({ name: revealName, type: revealType }) => {
            if (revealType === "toggle") {
              document.body.classList.toggle(
                revealName,
                value && settings[`theme:${revealName}`]
              );
            }
          });
        }
        ...
        categories.map((category, index) =>
          Spicetify.React.createElement(
            "button",
            {
              ...
              style: {
                width: `calc((100% - ${(categories.length - 1) * 8}px) / ${
                  categories.length
                })`,
  ...
  const ThemeSettings = ({ options }) => {
    const [settings, setSettings] = Spicetify.React.useState(() =>
      initialiseSettings(options)
    );
    const categoryRefs = Spicetify.React.useRef({});

    const SettingsChange = Spicetify.React.useCallback(
      (key, value) => {
        setSettings((prev) => {
          ...
          if (categoryOption?.revealOptions) {
            categoryOption.revealOptions.forEach((revealOption) => {
              newSettings[`theme:${revealOption.name}`] = value
                ? revealOption.defaultVal
                : false;
            });
          }
          return newSettings;
    ...
    const renderOption = Spicetify.React.useCallback(
    ...
    const RevealableOptions = Spicetify.React.useCallback(
      ({ option, settings, onSettingChange }) => {
      ...
      Spicetify.React.createElement(
        ...
        Spicetify.React.createElement(
          "button",
          {
            ...
            onClick: () => resetOptions(setSettings, options),
          },
          "Reset"
        ),
        Spicetify.React.createElement(
          "button",
          {
            ...
            onClick: () => saveOptions(settings, options),
          },
          "Save"
        )
      )
